...
- Walk the Org BoK index (`.claude/knowledge/org-bok/index.md`) and match against project-type tags, tech stack, and "use when…" guidance
...
- Degrade gracefully: a failed fetch becomes a "deep dive unavailable" note, never a stage failure
...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` — active-space guardrails and affirmed practices (read per `.claude/knowledge/aidlc-shared/rules-reading.md`)
2. `.claude/knowledge/aidlc-shared/` — methodology principles
3. `.claude/knowledge/aidlc-research-agent/` — agent-specific methodology (the precedent-research method)
...
Then, stage-specific: the Org BoK index at `.claude/knowledge/org-bok/index.md`, and ONLY the exemplar profiles the index match selects.

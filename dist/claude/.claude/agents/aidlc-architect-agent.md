...
Then, standing org defaults: the Org BoK architecture-principles guide at `.claude/knowledge/org-bok/guides/architecture-principles.md` (if exists) — org-wide architecture defaults that apply even when precedent-research was skipped, subject to the precedence rule the guide states.

Also read `.claude/knowledge/org-bok/patterns/INDEX.md` (if exists) and open only the pattern files relevant to the current task — the index, never the whole directory. A `blessed` pattern is the org-wide default for the decision it covers; a `draft` pattern is advisory, so verify before relying on it. An affirmed `team.md`/`project.md` rule naming the pattern overrides it for this space.

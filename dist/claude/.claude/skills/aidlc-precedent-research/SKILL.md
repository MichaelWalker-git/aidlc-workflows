---
name: aidlc-precedent-research
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `precedent-research` stage (ideation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage precedent-research --single`:
  the engine emits one run-stage directive for precedent-research and its gate, the
...
# AI-DLC Stage Runner — precedent-research

Run the `precedent-research` stage on its own. This is opt-in packaging over
`/aidlc --stage precedent-research --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage precedent-research --single
   ```

   The engine emits one `run-stage` directive for `precedent-research` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage precedent-research --result completed
   ```

...
- **Skill**: `.agents/skills/aidlc/` — Orchestrator (`SKILL.md`), stage protocol, and 33 stage files across 5 phase directories
- **Session skills** (read-only, user-invocable): `.agents/skills/aidlc-session-cost/`, `.agents/skills/aidlc-replay/`, `.agents/skills/aidlc-outcomes-pack/`, `.agents/skills/aidlc-distill/` — typed as `$aidlc-session-cost`, `$aidlc-replay`, `$aidlc-outcomes-pack`, `$aidlc-distill`. The three reporting skills pull every count from `bun .codex/tools/aidlc-runtime.ts summary --json` (no LLM-side counting). Classified `read-only`: they never advance the workflow stage pointer and never emit audit events. `aidlc-session-cost` and `aidlc-replay` print to the terminal only; `aidlc-outcomes-pack` writes `OUTCOMES.md`. `aidlc-distill` is the Org BoK authoring session: it gates the target repo against the curated allowlist (`bun .codex/tools/aidlc-utility.ts distill-check`) before any repo access, then drafts an exemplar profile, index entry, and guide additions for human curation — it writes only BoK drafts, never workflow state.

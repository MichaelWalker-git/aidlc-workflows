...
- The reference brief's UI directives, patterns to follow, and deep-dive pointers with the brief's file path (when the brief exists — omit when precedent-research was skipped). Pass the deep-dive fetch rule along verbatim: the subagent MAY fetch a pointed-at file with the session's ambient git credentials when it materially helps; a failed fetch degrades to a "deep dive unavailable" note, never a stage failure

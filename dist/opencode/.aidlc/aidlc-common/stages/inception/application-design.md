...
- Read the reference brief from `<record>/ideation/precedent-research/` (if exists) and follow its exemplar patterns — the selected precedent's architecture and rationale inform component boundaries and design decisions, subject to the brief's stated precedence rule. Cite the brief explicitly in `decisions.md` where it shaped a decision. If the brief is absent (the stage was skipped), proceed without it. When the brief's deep-dive pointers name a file that would materially help (a canonical module layout, the real service baseline), you MAY fetch it from the pointed-at repo using whatever git credentials the session already has; if the fetch fails or the repo is unreachable, note "deep dive unavailable" in the deliverable where the material would have helped and continue from the brief — a failed fetch is never a stage failure.

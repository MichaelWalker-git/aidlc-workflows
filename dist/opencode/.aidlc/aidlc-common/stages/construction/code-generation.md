...
- Reference brief from `<record>/ideation/precedent-research/reference-brief.md` (if exists) — follow its exemplar patterns, and apply its UI directives (design tokens, spacing and layout conventions, component patterns, copy tone) to all generated UI code so the output follows the org design language, subject to the brief's stated precedence rule. If the brief is absent (the stage was skipped), proceed without it. When the brief's deep-dive pointers name a file that would materially help (the real lint config to copy, a canonical component to imitate), you MAY fetch it from the pointed-at repo using whatever git credentials the session already has; if the fetch fails or the repo is unreachable, note "deep dive unavailable" in the deliverable where the material would have helped and continue from the brief — a failed fetch is never a stage failure.
...
- The reference brief's UI directives, patterns to follow, and deep-dive pointers with the brief's file path (when the brief exists — omit when precedent-research was skipped). Pass the deep-dive fetch rule along verbatim: the subagent MAY fetch a pointed-at file with the session's ambient git credentials when it materially helps; a failed fetch degrades to a "deep dive unavailable" note in the deliverable, never a stage failure

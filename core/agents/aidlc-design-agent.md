...
Then, standing org defaults: the Org BoK UI design-language guide at `{{HARNESS_DIR}}/knowledge/org-bok/guides/ui-design-language.md` (if exists) — the org's visual baseline (including the not-LLM-looking floor) that applies even when precedent-research was skipped, subject to the precedence rule the guide states.

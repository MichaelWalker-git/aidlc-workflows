...
Then, standing org defaults: the Org BoK architecture-principles guide at `{{HARNESS_DIR}}/knowledge/org-bok/guides/architecture-principles.md` (if exists) — org-wide architecture defaults that apply even when precedent-research was skipped, subject to the precedence rule the guide states.

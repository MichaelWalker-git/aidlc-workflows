...
Then, standing org defaults: the Org BoK code-style guide at `{{HARNESS_DIR}}/knowledge/org-bok/guides/code-style.md` (if exists) — org-wide style review criteria that apply even when precedent-research was skipped, subject to the precedence rule the guide states.

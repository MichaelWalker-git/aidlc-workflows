...
- **Skill**: `{{HARNESS_DIR}}/skills/aidlc/` — Orchestrator (`SKILL.md`), stage protocol, and 33 stage files across 5 phase directories
- **Session skills** (read-only, user-invocable): `{{HARNESS_DIR}}/skills/aidlc-session-cost/`, `{{HARNESS_DIR}}/skills/aidlc-replay/`, `{{HARNESS_DIR}}/skills/aidlc-outcomes-pack/`, `{{HARNESS_DIR}}/skills/aidlc-distill/` — typed as `{{INVOKE}}-session-cost`, `{{INVOKE}}-replay`, `{{INVOKE}}-outcomes-pack`, `{{INVOKE}}-distill`. The three reporting skills pull every count from `bun {{HARNESS_DIR}}/tools/aidlc-runtime.ts summary --json` (no LLM-side counting). Classified `read-only`: they never advance the workflow stage pointer and never emit audit events. `aidlc-session-cost` and `aidlc-replay` print to the terminal only; `aidlc-outcomes-pack` writes `OUTCOMES.md`. `aidlc-distill` is the Org BoK authoring session: it gates the target repo against the curated allowlist (`bun {{HARNESS_DIR}}/tools/aidlc-utility.ts distill-check`) before any repo access, then drafts an exemplar profile, index entry, and guide additions for human curation — it writes only BoK drafts, never workflow state.
...
- **Agents**: `{{HARNESS_DIR}}/agents/` — 15 agents: 12 domain-expert personas (product, research, design, delivery, architect, aws-platform, compliance, devsecops, developer, quality, pipeline-deploy, operations), 2 review-only agents (product-lead, architecture-reviewer), and the adaptive-workflows composer. {{SLOT:agents_note}}

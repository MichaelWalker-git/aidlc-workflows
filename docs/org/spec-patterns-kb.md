...
20. As a v1 reader, I want `multi-tenancy/jwt-tenant-isolation.md` to ship as `draft` now, so that I get multi-tenancy guidance immediately with calibrated trust rather than waiting on the Architect's SDP audit.
...
- **Backlog pattern content** beyond the four v1 files — serverless-compute, idp, genai, full-stack, eventing, iac topics are admitted incrementally later under the admission rule.
...
- **Blessing the multi-tenancy pattern** — it ships `draft`; the Architect's Service Delivery Platform audit later blesses or amends it, outside this spec.
